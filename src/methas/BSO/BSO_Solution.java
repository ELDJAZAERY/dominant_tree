...
import methas.GA.GA_Solution;

import java.util.Collections;
import java.util.Comparator;
import java.util.concurrent.ThreadLocalRandom;

public class BSO_Solution extends Binary_Solution {
    ...
    public BSO_Solution(){
        ...
        int i = 1;
        do{
            int fliper = ( (flip * i )% Graph.Nodes.size()-1 ) ;
            binary[fliper] = (byte) ( (binary[fliper] + 1) % 2) ;
            i++;
        }while (flip * i  < Graph.Nodes.size());
    ...
    public BSO_Solution(BSO_Solution sol , int numBee){
        ...
        int i = 1;
        do{
            int fliper = ( (flip * i + numBee)% Graph.Nodes.size()-1 ) ;
            binary[fliper] = (byte) ( (binary[fliper] + 1) % 2) ;
            i++;
        }while (flip * i + numBee < Graph.Nodes.size());
    ...
    public BSO_Solution(BSO_Solution sol){
        super();

        int flip = ThreadLocalRandom.current().nextInt(1,Graph.Nodes.size());
        BSO_Solution newSol = (BSO_Solution) sol.clone();
        Byte[] binary = newSol.binary;
    ...
    public int distance(){
        int dist = 0;
        for(int i = 0 ; i < binary.length ; i++ ) {
            if(binary[i] != BSO_Algo.CurrentSol.binary[i])
                dist++;
        }
        return dist;
    }
